...
class QuantAttentionQuantPattern(AttentionQuantPattern):
    ...
    def __init__(
        ...
        assert pre_quant_dtype == FP8_DTYPE
    ...
    def extra_check(self, match):
        ...
        logger.debug("Fused the pattern QuantAttentionQuantPattern for %s",
                     self.layer_name)
        return True
    ...
    def _register_fp8_quant(self, pm_pass: PatternMatcherPass):
        ...
        def replacement(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
            ...
            # reshape q
            q_view = RESHAPE_OP(q, [-1, self.num_heads * self.head_size])
            # quant q
            at1 = auto_functionalized(self.PRE_QUANT_OP,
                                      result=q_quant,
                                      input=q_view.contiguous(),
                                      scale=q_scale)
            # reshape q back
            q_quant_view = RESHAPE_OP(at1[1],
                                      [-1, self.num_heads, self.head_size])
            # attention
...
class AttnFusionPass(VllmInductorPass):
    ...
    def register_quant_attn_quant_pattern(self, config: VllmConfig):
        ...
        for layer_name, layer in get_layers_from_vllm_config(
            ...
            logger.debug("Successfully set o_proj.input_scale for %s",
                         layer_name)
            ...
            pattern_fp8 = QuantAttentionQuantPattern(layer,
