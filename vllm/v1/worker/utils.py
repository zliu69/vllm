...
def initialize_kv_cache_for_kv_sharing(
    ...
    layer_to_attn_group_idx: dict[str, int] = {}
    if attn_groups:
            ...
            for attn_group_idx, attn_group in enumerate(kv_attn_groups):
                for layer_name in attn_group.layer_names:
                    layer_to_kv_cache_group_idx[
                        layer_name] = kv_cache_group_idx
                    layer_to_attn_group_idx[layer_name] = attn_group_idx
    else:
        for kv_cache_group_idx, kv_cache_group in enumerate(kv_cache_groups):
            for layer_name in kv_cache_group.layer_names:
                layer_to_kv_cache_group_idx[layer_name] = kv_cache_group_idx

    for layer_name, target_layer_name in shared_kv_cache_layers.items():
        kv_caches[layer_name] = kv_caches[target_layer_name]
        kv_cache_group_idx = layer_to_kv_cache_group_idx[target_layer_name]
        kv_cache_groups[kv_cache_group_idx].layer_names.append(layer_name)

        if attn_groups:
            attn_group_idx = layer_to_attn_group_idx[target_layer_name]
