...
from anthropic.types.message_param import ImageBlockParam, TextBlockParam
...
def resolve_hf_chat_template(
    ...
    if tools is None:
        try:
            ...
            logger.debug("Failed to load AutoProcessor chat template for %s", tokenizer.name_or_path,
...
def _parse_chat_message_content_mm_part(
    ...
    # Handle anthropic image input
    if part_type == "image":
        if part.get("source", {}).get("type", "") == "base64":
            image_type = part.get("source", {}).get("media_type", "")
            source_type = part.get("source", {}).get("type", "")
            image_data = part.get("source", {}).get("data", "")
            return "image_url", f"data:{image_type};{source_type},{image_data}"
        if part.get("source", {}).get("type", "") == "url":
            return "image_url", part.get("source", {}).get("url", "")
